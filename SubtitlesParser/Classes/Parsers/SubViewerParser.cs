...
namespace SubtitlesParser.Classes.Parsers
{
    ...
    /// </summary>
    public partial class SubViewerParser : ISubtitlesParser
    {
        // Properties ----------------------------------------------------------
        ...
        [GeneratedRegex("\\d{2}:\\d{2}:\\d{2}\\.\\d{2},\\d{2}:\\d{2}:\\d{2}\\.\\d{2}", RegexOptions.Compiled)]
        ...
        {
            Assertions.ValidateStream(subStream);
                ...
                if (line != null && lineNumber <= MaxLineNumberForItems && IsTimestampLine(line))
                {
                    ...
                    while (line != null)
                    {
                        ...
                        if (IsTimestampLine(line))
                        {
                            ...
                            if (start > TimeSpan.Zero && end > TimeSpan.Zero && textLines.Any())
                            {
                    ...
                    if (lastStart > TimeSpan.Zero && lastEnd > TimeSpan.Zero && textLines.Any())
                    {
                        items.Add(new SubtitleItem()
                        {
                            StartTime = lastStart,
        ...
        private Tuple<TimeSpan, TimeSpan> ParseTimecodeLine(string line)
        {
        ...
        private TimeSpan ParseTimecode(string s)
        {
        ...
        private bool IsTimestampLine(string line)
        {
            ...
            var isMatch = TimestampRegex().IsMatch(line);
            return isMatch;
        }
    }
}
